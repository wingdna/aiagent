  ...
  try {
    ...
    try {
        const { data } = await supabase
            .from('agents')
            .select('id, name, description, entity_type, nri_score, hot_score, video_url, cover_url, display_mode, specs, pricing_model, pricing_model_json, metrics, capability_tags, media_gallery, vendor_id, vendor_slug, slogan')
            .ilike('name', `%${query}%`)
            .limit(8);
