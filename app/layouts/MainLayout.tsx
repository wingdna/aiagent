...
const MainLayout: React.FC<MainLayoutProps> = ({ initialAgents = [] }) => {
    const location = useLocation();
    const navigate = useNavigate();

    // ─── Global State & Hooks ────────────────────────────────────────────────
    const { keys, saveKey, rememberInSession, setRememberInSession } = useUserKeys();
    const { profile, addXp, updateBalance, logout } = useProfile();
    const { agents, initializing } = useAppData(initialAgents);
    const { activeAgentId, setActiveAgentId, showLogin, setShowLogin, showKeyVault, setShowKeyVault } = useUIStore();
    ...
    // Navigation & FX
    const { isTransit, direction, showGlitch, navWarning } = useNavigation(
        agents,
        activeAgentId,
        (id) => setActiveAgentId(id)
    );
