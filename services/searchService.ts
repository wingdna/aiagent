...
class SearchService {
    ...
    public async findSimilarAgents(agent: Agent, count: number = 8): Promise<Agent[]> {
        if (this.useDB && supabase) {
            // 1. Hybrid/Vector search via server proxy
            try {
                const queryText = `${agent.name} ${agent.category || ''} ${agent.capability_tags?.join(' ') || ''}`;
            ...
            try {
                const { data, error } = await supabase.from('agents').select(QUERY_FIELDS.AGENTS_MINIMAL).eq('category', agent.category).neq('id', agent.id).limit(count);
                if (!error && data) return data as unknown as Agent[];
            } catch (_) { }
        }
        return this.localCache.filter(a => a.category === agent.category && a.id !== agent.id).slice(0, count);
    }
