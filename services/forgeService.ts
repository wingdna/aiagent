...
export const forgeIdentity = async (agent: Agent): Promise<string | null> => {
    console.log(`[FORGE] Initiating sequence for: ${agent.name} (${agent.category})`);
    
    // Key check is now handled implicitly by the Relay or checked before calling
    const storedKeys = sessionStorage.getItem('YOUAGENT_KEYS');
    ...
    try {
        ...
        if (imageUrl) {
            ...
            if (CONFIG.USE_DATABASE && supabase) {
                 ...
                 console.log(`[FORGE] SUCCESS: Database updated for ${agent.name}`);
            }
            return imageUrl;
        }
    } catch (e) {
