...
class AgentQueryService {
    ...
    private _mapAgentRowToAgent(row: AgentRow): Agent {
        return {
            ...
            nri_score: row.nri_score || undefined,
            ...
            pricing_model_json: row.pricing_model_json as any,
            ...
            pricing_model: row.pricing_model ? { type: row.pricing_model } : undefined,
            pricing_details: row.pricing_details || undefined,
    ...
    public async getAgents(page: number = 0, limit: number = 20, category: string = 'ALL', sortBy: string = 'hot', order: string = 'desc'): Promise<Agent[]> {
        ...
        try {
            let query = supabase.from('agents').select(QUERY_FIELDS.AGENTS_FULL);
