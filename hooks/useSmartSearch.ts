...
export const executeSemanticSearch = async (query: string) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('agents')
      .select('id, slug, name, description, entity_type, nri_score, hot_score, video_url, cover_url, display_mode, specs, pricing, metrics, capability_tags, media_gallery, vendor_id, vendor_slug, slogan, technical_specs, social_proof, external_stats, framework_stack, developer_socials')
      .ilike('name', `%${query}%`)
      .limit(12);
