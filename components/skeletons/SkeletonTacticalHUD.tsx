...
export const SkeletonTacticalHUD: React.FC = () => {
    return (
        <div className="relative w-full h-full flex flex-col font-sans bg-black bg-topology overflow-hidden">
            ...
            {/* Right Rail Skeleton (Sync with ActionBar) */}
            <div className="fixed right-2 md:right-6 top-1/2 -translate-y-1/2 z-[100] scale-75 md:scale-100 origin-right w-[60px] flex flex-col items-center gap-6 p-4 bg-[#050505]/80 backdrop-blur-xl border border-white/10 rounded-2xl">
                {[1, 2, 3, 4].map(i => (
                    <div key={i} className="w-6 h-6 rounded bg-gray-800 animate-pulse"></div>
                ))}
            </div>
