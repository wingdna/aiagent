        ...
        <div className="h-full w-full bg-black flex flex-col relative pt-20">
            ...
            <div className="px-6 pb-4 border-b border-gray-900 bg-black/80 backdrop-blur z-20 flex justify-between items-end">
                ...
                <div className="flex items-center gap-4">
                    {mode === 'SOLO' && (
                        <div className="flex items-center gap-1 bg-gray-900 rounded p-1 border border-gray-800 mr-2">
                            <button onClick={() => adjustSoloCount(-1)} disabled={soloSlotCount <= 2 || battleState !== 'IDLE'} className="p-1 hover:bg-gray-800 rounded disabled:opacity-30" aria-label="Decrease slots"><X size={12} /></button>
                            <span className="text-xs font-bold text-white w-4 text-center">{soloSlotCount}</span>
                            <button onClick={() => adjustSoloCount(1)} disabled={soloSlotCount >= 4 || battleState !== 'IDLE'} className="p-1 hover:bg-gray-800 rounded disabled:opacity-30" aria-label="Increase slots"><Plus size={12} /></button>
                        </div>
                    )}
            ...
            {/* --- MAIN CONTENT AREA --- */}
            <div className="flex-1 relative overflow-y-auto md:overflow-hidden bg-black pb-20 md:pb-0">
                {mode === 'TACTICAL' ? (
                    <TacticalModePanel
                        agents={agents}
