                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 overflow-y-auto pb-10">
                        {agents.map(agent => (
                            <div key={agent.id} onClick={() => addNode(agent)} className="border border-gray-800 bg-gray-900/50 p-4 rounded hover:border-cyan-400 hover:bg-cyan-500/10 cursor-pointer flex items-center gap-4 transition-all">
                                <img src={agent.video_poster} alt="" className="w-12 h-12 rounded object-cover" loading="lazy" decoding="async" />
                                <div>
                        ...
                        <AnimatePresence>
                            {workflowNodes.map((node, i) => (
                                <React.Fragment key={node.id + i}>
                                    ...
                                    >
                                        <img src={node.video_poster} alt="" className="w-full h-24 object-cover opacity-50 group-hover:opacity-80 transition-opacity" loading="lazy" decoding="async" />
