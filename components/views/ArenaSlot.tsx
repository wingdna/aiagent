        ...
        >
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        {getAgentPool().map(a => (
                            <div 
                                ...
                                className="flex items-center gap-3 p-2 border border-gray-800 hover:border-cyan-400 hover:bg-cyan-500/10 cursor-pointer rounded transition-all group"
                            >
                                <img src={a.video_poster} className="w-8 h-8 rounded object-cover grayscale group-hover:grayscale-0" loading="lazy" decoding="async" />
            ...
            {agent ? (
                <>
                    ...
                    {hasKey && (
                        <>
                            <img src={agent.video_poster} className="absolute inset-0 w-full h-full object-cover opacity-20 mix-blend-screen grayscale" loading="lazy" decoding="async" />
                            ...
                            {!hasKey ? (
                                ...
                                <div className="text-center">
                                    ...
                                    <div className="flex justify-center gap-3 text-[9px] font-mono text-gray-400">
                                        ...
                                        <span>SPD: {agent.metrics?.speed || 0}</span>
                                    </div>
                                </div>
                            )}
                        </div>
                                ...
                                >
                                    {logs.map((log, i) => (
                                        <div key={i} className="mb-1 break-words">
                                            ...
                                            <span className={log.includes('OUTPUT') ? 'text-cyan-400' : 'text-yellow-500/80'}>{log}</span>
                                        </div>
                                    ))}
                                    {isProcessing && <span className="text-cyan-400 animate-pulse">_</span>}
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div onClick={handleSlotClick} className="flex-1 flex flex-col items-center justify-center cursor-pointer group">
                    ...
                    <span className="text-[10px] font-mono text-gray-600 group-hover:text-cyan-400">{label}</span>
