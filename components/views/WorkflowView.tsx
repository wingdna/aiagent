...
export const WorkflowEngine: React.FC<{ agents: Agent[] }> = ({ agents }) => {
    ...
    return (
        <div className="h-full w-full bg-black flex flex-col pt-20 px-6 pb-6 overflow-hidden">
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-6 h-auto md:h-full overflow-y-auto md:overflow-hidden pb-20 md:pb-0">
                
                {/* 1. NODE SELECTOR */}
                <div className="col-span-12 md:col-span-3 bg-gray-900/30 border border-gray-800 rounded-xl p-4 flex flex-col h-[400px] md:h-full overflow-hidden">
                ...
                <div className="col-span-12 md:col-span-9 flex flex-col h-auto md:h-full gap-6">
                    ...
                    <div className="h-48 bg-black/50 border border-gray-800 rounded-xl p-6 relative flex items-center justify-start gap-4 overflow-x-auto">
                        ...
                        <div className="shrink-0 w-32 h-32 border-2 border-dashed border-gray-700 rounded-lg flex items-center justify-center relative bg-gray-900/20">
                            ...
                            {chainState.status === 'RUNNING' && chainState.step === 0 && (
                                <m.div layoutId="packet" className="absolute w-4 h-4 bg-white rounded-full shadow-[0_0_10px_white]" />
                            )}
                        </div>
                        ...
                        {selectedAgents.map((agent, i) => (
                            <React.Fragment key={agent.id}>
                                ...
                                <div className={`shrink-0 w-32 h-32 border border-gray-700 rounded-lg flex flex-col items-center justify-center relative transition-all ${chainState.step === i + 1 ? 'border-blue-500 bg-blue-900/10 shadow-[0_0_20px_rgba(59,130,246,0.2)]' : 'bg-gray-900/40'}`}>
                                    <img src={agent.video_poster} className="w-8 h-8 rounded-full mb-2 object-cover opacity-80" loading="lazy" decoding="async" />
                                    ...
                                    {chainState.status === 'RUNNING' && chainState.step === i + 1 && (
                                         <m.div layoutId="packet" className="absolute w-full h-full border-2 border-blue-500 rounded-lg" initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} />
                                    )}
                                </div>
                            </React.Fragment>
                        ))}
