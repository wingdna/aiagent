...
export const DiscoverView: React.FC<DiscoverViewProps> = ({
    ...
    useEffect(() => {
        ...
        targets.forEach(agent => {
            ...
            const posterUrl = agent.video_poster || agent.persona_img;
            if (posterUrl) {
                const img = new Image();
            ...
            if (agent.video_url && (agent.video_url.includes('youtube') || agent.video_url.includes('youtu.be'))) {
                const thumb = getYouTubeThumbnail(agent.video_url, 'max');
                if (thumb) {
                    const img = new Image();
                    img.src = thumb;
                    // @ts-ignore
    ...
    const coverUrl = currentAgent ? sanitizeValue(currentAgent.cover_url || fallbackImage) : fallbackImage;
        ...
        <div className="relative h-full w-full bg-black">
            ...
            <div className={`absolute inset-0 z-0 transition-all duration-1000 ${isSystemCalculationMode ? 'grayscale brightness-50' : ''}`}>
                {currentAgent && (
                    ...
                    >
                        <img
                            src={getBgImage(currentAgent.video_poster)}
                            alt="bg"
