    ...
    return (
        <div className="min-h-screen bg-[#050505] text-white font-mono">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-[2fr_4fr_1fr] gap-4 p-6 text-xs text-gray-500 uppercase tracking-widest border-b border-white/5 sticky top-[88px] bg-[#050505]/90 backdrop-blur-md z-10" role="row">
                    <div role="columnheader">Agent Entity</div>
                    <div role="columnheader">Intelligence Summary</div>
                ...
                <div className="divide-y divide-white/5" role="rowgroup">
                    {agents.map((agent, index) => (
                        <div 
                            ...
                            className="grid grid-cols-[2fr_4fr_1fr] gap-4 p-4 items-center hover:bg-white/5 transition-colors"
                            role="row"
                        >
                            <Link to={`/agent/${agent.slug || agent.id}`} className="text-cyan-400 font-bold truncate" aria-label={`View details for ${agent.name}`}>
                                {agent.name}
                            </Link>
                            <div className="text-gray-400 truncate text-sm" title={agent.description || agent.slogan || ''}>
