...
export const LeaderboardView: React.FC = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            const sorted = allAgents.sort((a, b) => (b.hot_score || 0) - (a.hot_score || 0));
    ...
    const getTrend = (id: string, currentRank: number) => {
        ...
        if (!prev) return <span className="text-matrix-green text-[9px] font-bold">NEW</span>;
        const diff = prev - currentRank;
        if (diff > 0) return <div className="flex items-center text-matrix-green text-[9px] font-bold"><TrendingUp size={10} /> {diff}</div>;
    ...
    return (
        <div className="h-screen w-full bg-black flex flex-col pt-20 overflow-hidden relative">
            ...
            {/* Table Header */}
            <div className="grid grid-cols-12 px-6 py-2 border-b border-gray-800 text-[10px] font-mono text-gray-500 uppercase tracking-widest bg-gray-900/30">
                <div className="col-span-1 text-center">RANK</div>
                <div className="col-span-1 text-center">TREND</div>
                <div className="col-span-4">AGENT_ENTITY</div>
                <div className="col-span-2">CATEGORY</div>
                <div className="col-span-2 text-right">NRI_SCORE</div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pb-10">
                {loading ? (
                    <div className="p-10 text-center text-matrix-green font-mono animate-pulse">
                    ...
                    return (
                        <motion.div 
                            ...
                            className={`grid grid-cols-12 px-6 py-4 border-b border-gray-900 items-center hover:bg-gray-900/30 transition-colors group ${glow} border-l-4`}
                        >
                            <div className="col-span-1 text-center font-display font-bold text-lg text-white">
                            ...
                            <div className="col-span-4 flex items-center gap-4">
                                <div className="w-10 h-10 rounded overflow-hidden border border-gray-800 relative group-hover:border-white transition-colors">
                                    <img src={agent.video_poster} className="w-full h-full object-cover" />
                                </div>
                                <div>
                                    <div className="text-sm font-bold text-white group-hover:text-matrix-green transition-colors">{agent.name}</div>
                            ...
                            <div className="col-span-2 text-right font-mono font-bold text-white text-lg">
                                {agent.hot_score?.toFixed(1) || '---'}
                            </div>
                            <div className="col-span-2 text-right font-mono text-xs text-gray-400">
