...
import { FileText, Rss, ArrowUpRight, Zap, RefreshCw, ChevronRight } from 'lucide-react';
        ...
        <AnimatePresence mode="popLayout">
            {shouldShow && (
                ...
                >
                    ...
                    <div className="p-0 md:p-2 space-y-1 relative">
                        {loading ? (
                            ...
                            intelItems.map((item, idx) => {
                                const hasInternalId = !!item.id;
                                const hasUrl = !!item.url;
                                const isClickable = hasInternalId || hasUrl;
                                ...
                                const content = (
                                    <>
                                        ...
                                        <div className="flex items-center gap-2 w-full">
                                            {/* Icon */}
                                            <div className={`opacity-60 transition-opacity relative z-10 shrink-0 ${isClickable ? 'group-hover/item:opacity-100' : ''}`}>
                                            ...
                                            <div className="flex items-center gap-2 shrink-0 relative z-10">
                                                <span className="text-[9px] font-mono text-gray-600 whitespace-nowrap">
                                                    <span className="md:hidden">({item.date?.replace(' ago', '') || item.published_at?.split('T')[0]})</span>
                                                    <span className="hidden md:inline">{item.date || item.published_at?.split('T')[0]}</span>
                                                </span>
                                                {hasInternalId ? (
                                                    <ChevronRight size={14} className="text-gray-600 group-hover/item:text-cyan-400 transition-colors opacity-0 group-hover/item:opacity-100 hidden md:block" />
                                                ) : hasUrl ? (
                                ...
                                if (hasInternalId) {
                                    ...
                                    return (
                                        <MotionLink
                                            key={idx}
                                            to={`/blog/intel-${item.id}`}
                                            initial={{ x: -20, opacity: 0 }}
                                            animate={{ x: 0, opacity: 1 }}
                                            transition={{ delay: idx * 0.1 }}
                                            className={`group/item flex flex-col gap-1 p-2 md:p-3 rounded-lg transition-all border border-transparent relative overflow-hidden hover:bg-cyan-500/10 cursor-pointer hover:border-cyan-500/30 ${idx > 1 ? 'hidden md:flex' : 'flex'}`}
                                        >
                                            {content}
                                        </MotionLink>
                                    );
                    ...
                    <div className="p-2 bg-black/80 border-t border-white/5 text-center rounded-b-xl">
                        <Link to="/blog" className="text-[9px] font-mono text-gray-500 hover:text-cyan-400 cursor-pointer transition-colors uppercase">VIEW_FULL_INTELLIGENCE_REPORT {">>"}</Link>
                    </div>
                </m.div>
            )}
        </AnimatePresence>
