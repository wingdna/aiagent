...
export const IntelPreview: React.FC<IntelPreviewProps> = ({ agent, accentColor }) => {
    ...
    return (
        <div className="w-full max-w-2xl md:bg-black/60 md:border md:border-white/10 md:backdrop-blur-md md:rounded-xl md:shadow-2xl relative group min-h-[160px]">
                        ...
                        return (
                            <Wrapper 
                                ...
                                className={`group/item flex flex-col gap-1 p-2 md:p-3 rounded-lg transition-all border border-transparent relative overflow-hidden ${
                                    hasUrl 
                                    ? 'hover:bg-matrix-green/10 cursor-pointer hover:border-matrix-green/30' 
                                    : 'bg-black/20 border-white/5'
                                } ${idx > 1 ? 'hidden md:flex' : 'flex'}`}
                            >
                                {/* Hover Sweep Effect (Only if clickable) */}
                                {hasUrl && (
                                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-matrix-green/5 to-transparent translate-x-[-100%] group-hover/item:translate-x-[100%] transition-transform duration-700 ease-in-out pointer-events-none"></div>
                                )}
                                ...
                                <div className="flex items-center gap-2 w-full">
                                    ...
                                    <div className="flex-1 min-w-0 relative z-10 flex items-center gap-2">
                                        ...
                                        <h4 className={`flex-1 min-w-0 text-xs md:text-sm font-mono leading-tight truncate ${
                                            hasUrl 
                                            ? 'font-bold text-gray-200 group-hover/item:text-white group-hover/item:underline decoration-matrix-green/50 decoration-2 underline-offset-2 transition-colors' 
                                    ...
                                    <div className="flex items-center gap-2 shrink-0 relative z-10">
                                        ...
                                        {hasUrl && (
                                            <ArrowUpRight size={14} className="text-gray-600 group-hover/item:text-matrix-green transition-colors opacity-0 group-hover/item:opacity-100 hidden md:block" />
                                        )}
                                    </div>
                                </div>
            ...
            <div className="p-2 bg-black/80 border-t border-white/5 text-center hidden md:block rounded-b-xl">
                <span className="text-[9px] font-mono text-gray-500 hover:text-matrix-green cursor-pointer transition-colors">VIEW_FULL_INTELLIGENCE_REPORT {">>"}</span>
            </div>
        </div>
    );
};
