...
import { 
  ...
  Search, Key, LogIn, LogOut, Shield, User, BookOpen, List 
} from 'lucide-react';
...
export const CommandRail: React.FC<CommandRailProps> = ({
    ...
    const isActive = (path: string) => {
        if (path === '/') {
            return location.pathname === '/' || (location.pathname.startsWith('/agent/') && !location.pathname.endsWith('/lounge'));
        }
    ...
    return (
        <>
                        ...
                        <div className="relative group">
                            <button 
                                ...
                                className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-500 border overflow-hidden ${
                                    isLoggedIn ? 'border-cyan-500/50 bg-cyan-500/5' : 'border-gray-800 bg-gray-900/50 hover:border-white/30'
                                }`}
                            >
                                {isLoggedIn ? (
            ...
            <nav className="fixed bottom-0 left-0 right-0 h-16 bg-black/80 backdrop-blur-xl border-t border-white/10 z-[100] md:hidden flex items-center justify-around px-4">
                <Link to="/" className={`p-3 ${isActive('/') ? 'text-cyan-400' : 'text-gray-500'}`}><Compass size={24} /></Link>
                <Link to="/battle" className={`p-3 ${isActive('/battle') ? 'text-red-500' : 'text-gray-500'}`}><Swords size={24} /></Link>
                <button onClick={onToggleCommander} className={`p-3 ${isCommanderOpen ? 'text-cyan-400 animate-pulse' : 'text-gray-500'}`}><Search size={24} /></button>
                <Link to="/directory" className={`p-3 ${isActive('/directory') ? 'text-green-500' : 'text-gray-500'}`}><List size={24} /></Link>
                <Link to="/workflow" className={`p-3 ${isActive('/workflow') ? 'text-blue-500' : 'text-gray-500'}`}><Layers size={24} /></Link>
                <button onClick={isLoggedIn ? onLogoutClick : () => setShowLogin(true)} className={`p-3 ${isLoggedIn ? 'text-cyan-400' : 'text-gray-500'}`}>{isLoggedIn ? <Shield size={24} /> : <LogIn size={24} />}</button>
            </nav>
        </>
    );
};
