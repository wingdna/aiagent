...
export const CerebroEngine: React.FC = () => {
    ...
    const fetchStats = async () => {
        ...
        // PENDING
        const { count: pending } = await supabase.from('seed_queue').select('*', { head: true, count: 'exact' }).eq('status', 'PENDING');
        // COMPLETED (Agents Live)
        const { count: live } = await supabase.from('agents').select('*', { head: true, count: 'exact' });
        // FAILED (Network/API)
        const { count: network } = await supabase.from('seed_queue').select('*', { head: true, count: 'exact' }).eq('status', 'FAILED');
        // ERROR_CONTENT (URL Alive but unusable)
        const { count: content } = await supabase.from('seed_queue').select('*', { head: true, count: 'exact' }).eq('status', 'ERROR_CONTENT');
    ...
    return (
        <div className="flex flex-col h-full bg-black/50 rounded-xl border border-matrix-green/20 overflow-hidden">
            ...
            <div className="flex border-b border-gray-800 bg-black/60 font-mono text-[9px] text-gray-400">
                ...
                <div className="flex-1 p-2 border-r border-gray-800 flex flex-col items-center justify-center">
                    ...
                    <span className="text-sm font-bold text-matrix-green">{stats.completedAgents}</span>
                </div>
                <div className="flex-1 p-2 border-r border-gray-800 flex flex-col items-center justify-center">
            ...
            {/* Navigation Tabs */}
            <div className="flex border-b border-matrix-green/20 bg-black/40">
                {UNITS.map(u => (
                    <button key={u.id} onClick={() => setActiveUnit(u.id)} className={`flex-1 p-3 text-[10px] font-mono font-bold flex flex-col items-center justify-center gap-1 transition-all border-b-2 ${activeUnit === u.id ? 'border-matrix-green bg-matrix-green/10 text-matrix-green' : 'border-transparent text-gray-500 hover:text-white'}`}>
                        <u.icon size={16} />
            ...
            <div className="flex-1 p-4 flex flex-col gap-4 overflow-hidden relative">
                {activeUnit === 'UNIT_01' && (
                    <div className="flex flex-col gap-4 h-full">
                        <textarea value={sourceUrl} onChange={(e) => setSourceUrl(e.target.value)} className="flex-1 bg-black border border-gray-700 rounded p-3 text-xs font-mono text-white focus:border-matrix-green outline-none resize-none" placeholder="Enter source URL (GitHub README, News feed...)" />
                        <button onClick={() => handleHarvest(sourceUrl)} disabled={isHarvesting} className="p-4 bg-matrix-green/20 border border-matrix-green/50 text-matrix-green hover:bg-matrix-green hover:text-black font-bold font-mono text-xs rounded transition-all flex items-center justify-center gap-2">
                ...
                {activeUnit === 'UNIT_02' && (
                    <div className="flex flex-col gap-6 h-full justify-center items-center">
                        <div className="flex gap-4 items-center">
                            ...
                            <button onClick={toggleDigest} disabled={cooldown > 0} className={`h-16 px-10 font-bold font-display text-sm tracking-widest rounded border transition-all ${isDigesting ? 'bg-red-500/20 text-red-500 border-red-500' : 'bg-matrix-green/20 text-matrix-green border-matrix-green'}`}>
                ...
                <div className="h-32 bg-black border border-gray-800 rounded p-2 font-mono text-[9px] text-gray-300 overflow-y-auto custom-scrollbar relative flex flex-col mt-auto">
                    ...
                    {logs.map((log, i) => (
                        <div key={i} className={`mb-1 break-words ${log.includes("FATAL") ? "text-red-400" : log.includes("SUCCESS") ? "text-matrix-green" : ""}`}>
                            {log}
                        </div>
                    ))}
                    <div ref={logEndRef} />
