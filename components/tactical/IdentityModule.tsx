...
export const IdentityModule: React.FC<IdentityModuleProps> = ({
    ...
    const rating = agent.nri_score ?? agent.metrics?.nri_score ?? 85;
    ...
    return (
        <div className="flex flex-col relative h-full">
            ...
            <header className="flex flex-wrap items-center gap-2 mb-4 relative z-10">
                <h2 className="flex items-center gap-1.5 px-3 py-1 rounded border border-white/10 bg-black/40 backdrop-blur-sm">
            ...
            <div className="mb-6 relative z-10 bg-black/20 p-4 rounded-lg border border-white/5 backdrop-blur-sm">
                <div className="grid grid-cols-[24px_1fr_30px] gap-y-3 items-center">
                    ...
                    <div className="flex items-center">
                        <AeroStarRating rating={agent.metrics?.reasoning || 0} />
                    </div>
                    <data value={agent.metrics?.reasoning || 0} className="text-xs font-mono text-cyan-500 text-right font-bold">
                        {agent.metrics?.reasoning || 0}
                    </data>
                    ...
                    <div className="flex items-center">
                        <AeroStarRating rating={agent.metrics?.creativity || 0} />
                    </div>
                    <data value={agent.metrics?.creativity || 0} className="text-xs font-mono text-cyan-500 text-right font-bold">
                        {agent.metrics?.creativity || 0}
                    </data>
                    ...
                    <div className="flex items-center">
                        <AeroStarRating rating={agent.metrics?.speed || 0} />
                    </div>
                    <data value={agent.metrics?.speed || 0} className="text-xs font-mono text-cyan-500 text-right font-bold">
            ...
            <div className="flex flex-wrap gap-3 my-4 pl-1 relative z-10">
              {/* NRI: Cold Authority */}
              <div title="NRI_AUTH" aria-label={`NRI Authority Score: ${Math.round(agent.nri_score ?? agent.metrics?.nri_score ?? 0)}`} className="cursor-help flex items-center gap-2 px-3 py-1.5 bg-slate-900 border border-slate-700 rounded-md hover:border-cyan-500/50 transition-colors">
                <Shield size={14} className="text-cyan-500" aria-hidden="true" />
                <data value={Math.round(agent.nri_score ?? agent.metrics?.nri_score ?? 0)} className="text-slate-200 font-bold font-mono text-sm">
                    {Math.round(agent.nri_score ?? agent.metrics?.nri_score ?? 0)}
                </data>
              </div>
              {/* HOT: Kinetic Heat */}
              <div title="HOT_KINETIC" aria-label={`Kinetic Heat Score: ${Number((agent.hot_score ?? agent.metrics?.hot_score ?? 0).toFixed(1))}`} className="cursor-help flex items-center gap-2 px-3 py-1.5 bg-red-950/30 border border-red-900/50 rounded-md hover:border-red-500/50 transition-colors">
                <Flame size={14} className="text-red-500" aria-hidden="true" />
                <data value={Number((agent.hot_score ?? agent.metrics?.hot_score ?? 0).toFixed(1))} className="text-red-400 font-bold font-mono text-sm">
                    {Number((agent.hot_score ?? agent.metrics?.hot_score ?? 0).toFixed(1))}
                </data>
              </div>
            </div>
            {/* ----------------------------------------- */}
                ...
                <section className="mb-6 relative z-10">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {agent.framework_stack.map((tech, idx) => (
                            <span key={idx} className="px-2 py-1 bg-cyan-950/20 border border-cyan-500/20 rounded text-[9px] font-mono text-cyan-300 flex items-center gap-1 hover:bg-cyan-900/30 transition-colors cursor-default">
                                {tech}
                            </span>
                        ))}
                    </div>
