...
const TierCard: React.FC<{ tier: any, idx: number, total: number }> = ({ tier, idx, total }) => {
    ...
    if (typeof priceDisplay === 'string') {
        // Remove double dollar signs and other formatting issues
    ...
    // Determine if it's truly free
    const isFree = priceDisplay === 0 || priceDisplay === '0' || priceDisplay === 'FREE' || priceDisplay === '$0';
    const tierNameLower = (tier.name || '').toLowerCase();
    const isOSS = tierNameLower.includes('open') || tierNameLower.includes('oss') || tierNameLower.includes('mit') || tierNameLower.includes('apache');
    
    const isActuallyFree = isFree || isOSS;
            ...
            <div className="flex justify-between items-center mb-2 relative z-10 h-8">
                <div className="flex items-center">
                    <span className="text-xl text-cyan-400 font-bold tracking-tight font-sans">
                        {isActuallyFree ? 'FREE' : (priceDisplay === undefined || priceDisplay === null ? '待求证' : (typeof priceDisplay === 'number' ? `$${priceDisplay}` : priceDisplay))}
                    </span>
                    {originalPrice && (
...
export const PricingMatrix: React.FC<PricingMatrixProps> = ({ agent }) => {
  const pricing = agent.pricing?.details || {};
  const pricingJson = agent.pricing?.details || {};
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      {agent.pricing?.price_text && (
          <div className="bg-white/5 border border-white/10 rounded-lg p-3 text-xs text-white font-mono mb-3">
             {agent.pricing.price_text}
          </div>
      )}
      ...
      {!agent.pricing?.price_text && !['Subscription', 'Closed-SaaS', 'API_Usage', 'Free_OpenSource', 'Open-Weights'].includes(type as string) && (
