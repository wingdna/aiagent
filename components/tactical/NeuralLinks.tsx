...
export const NeuralLinks: React.FC<{ agent: Agent }> = ({ agent }) => {
    const [linkedEntities, setLinkedEntities] = useState<any[]>([]);
    const [semanticLinks, setSemanticLinks] = useState<Agent[]>([]);
    ...
    useEffect(() => {
        ...
        const fetchSemanticLinks = async () => {
            ...
            try {
                ...
                // Filter out self
                setSemanticLinks(matches.filter(a => a.id !== agent.id).slice(0, 4));
            } catch (e) {
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                ...
                                <div 
                                    ...
                                    onClick={(e) => {
                                        ...
                                        navigate(`/agent/${identifier}/lounge`);
                            ...
                            {isScanningNeural ? (
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {semanticLinks.map(link => (
                                        ...
                                        >
                                            <img src={link.video_poster || link.cover_url} className="w-8 h-8 rounded object-cover opacity-70 group-hover:opacity-100" />
                                            <div className="flex flex-col overflow-hidden">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <div className="w-16 h-1 bg-gray-800 rounded-full overflow-hidden">
                                                        <div className="h-full bg-purple-500" style={{ width: `${(link.similarity || 0) * 100}%` }}></div>
                                                    </div>
                                                    <span className="text-[9px] font-mono text-gray-500">{((link.similarity || 0) * 100).toFixed(0)}%</span>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
